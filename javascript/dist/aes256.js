...
const AES256 = {
    encrypt(input, passphrase, format = 'concat') {
        return CryptoJS.AES.encrypt(input, passphrase, {format: ConcatFormatter}).toString();
    },

    decrypt(crypted, passphrase, format = 'concat') {
        return CryptoJS.AES.decrypt(crypted, passphrase, {formatter: format.toLowerCase() == 'concat' ? ConcatFormat : JsonFormat}).toString(CryptoJS.enc.Utf8);
    }
};
