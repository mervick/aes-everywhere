...
string decryptFinal(unsigned char *ciphertext, int ciphertext_len, unsigned char *key, unsigned char *iv)
{
    EVP_CIPHER_CTX *ctx;
...
string encryptFinal(unsigned char *text, int text_len, unsigned char *key, unsigned char *iv)
{
    EVP_CIPHER_CTX *ctx;
    unsigned char *plaintexts;
    int len;
...
string encrypt(string text, string passphrase)
{
    ...
    do {
        ...
        string key = salted.substr(0, 32);
        string iv = salted.substr(32, 16);
    }
    while ((key.length() != 32 || iv.length() != 16) && j--);
...
{
    cout << encrypt("text", "pass") << endl;
    string enc = encrypt("text", "pass");
//    cout << decrypt(enc, "pass") << endl;
    cout << decrypt("U2FsdGVkX19Raqa8dz7ncfv0lm4XgSCSpl4PB2vN3UA=", "pass") << endl;
    return 0;
}
